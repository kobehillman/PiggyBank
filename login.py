...
def register():
    login.destroy()
    subprocess.call([sys.executable, 'register.py'])
...
def hide():
    global button_mode
    if button_mode:
        eye_button.configure(image=closedeye, text="", fg_color="#343434", hover_color="#343434")
        password_entry.configure(show="*")
        button_mode = False
    else:
        eye_button.configure(image=openeye, text="", fg_color="#343434", hover_color="#343434")
        password_entry.configure(show="")
        button_mode = True
    ...
    if (username == "" or username == "Username") or (password == "" or password == "Password"):
        ...
        if myresult is None:
            ...
            try:
                ...
                CTkMessagebox(title="Error", message="An error occurred during login.", icon='warning')
...
if __name__ == '__main__':
    ...
    # (Using CTkButton for consistency with CustomTkinter)
    eye_button = ctk.CTkButton(login, image=closedeye, text="", fg_color="#343434",corner_radius=90,
                               hover_color="#343434", command=hide, width=5, height=5)
    eye_button.place(x=260, y=300)

    # CREATES AND PLACES THE LOGIN BUTTON
    login_button = ctk.CTkButton(login, text="Login",
                                 font=("Arial", 20), fg_color="#ffb4bb", hover_color="#fd5465", text_color='black',
    ...
    register_button = ctk.CTkButton(login, text="Register",
                                    font=("Arial", 20), fg_color="#ffb4bb", text_color='black',
                                    hover_color="#fd5465", command=register)
