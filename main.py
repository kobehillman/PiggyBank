...
class HomePage(ctk.CTkFrame):
    ...
    # def load_transactions(self):
    #     # Placeholder for future transaction display
    #     for widget in self.transactions_frame.winfo_children():
    #         widget.destroy()
    #
    #     label = ctk.CTkLabel(self.transactions_frame, text="Recent transactions will appear here.", font=("Arial", 14))
    #     label.pack(pady=10)
# class HomePage(ctk.CTkFrame):
#     def __init__(self, parent, controller):
#         super().__init__(parent, fg_color="#2e2e2e")
#         self.controller = controller
#
#         # Force frame size to match right-side container
#         self.configure(width=900, height=500)
#         self.pack_propagate(False)
#
#         # Title
...
#         title.pack(pady=20, fill="x")
#
#         # Frame to hold account cards
#         self.accounts_frame = ctk.CTkFrame(self, fg_color="#3a3a3a")
#         self.accounts_frame.pack(pady=10, padx=20, fill="both", expand=True)
#
#         # Frame to display recent transactions
#         self.transactions_frame = ctk.CTkFrame(self, fg_color="#3a3a3a")
#         self.transactions_frame.pack(pady=10, padx=20, fill="both", expand=True)
#
...
#             widget.destroy()
#
#         if not self.controller.current_username:
#             return
#
...
#             self.create_account_card(account_type, balance)
#
#     def fetch_user_accounts(self):
#         """Fetch account balances for the current user and decrypt them"""
#         conn = sqlite3.connect('bankapp.db')
#         cursor = conn.cursor()
#         cursor.execute("""
#             SELECT account_type, balance
...
#
#         accounts = {}
#         for acc_type, balance_enc in rows:
#             balance = float(decrypt_balance(balance_enc))
...
#             card,
#             text=f"{account_type.capitalize()} Account",
#             font=("Arial", 16, "bold")
#         )
#         account_label.pack(anchor="w", padx=15, pady=(10, 0))
#
...
#     # ------------------ DEPOSIT / WITHDRAW ------------------
#     def deposit(self, account_type):
#         amount = CTkInputDialog(text=f"Enter deposit amount for {account_type}:").get_input()
#         if not amount:
#             return
#         try:
#             amount = float(amount)
...
#         new_balance = current_balance + amount
#         cursor.execute("UPDATE accounts SET balance=? WHERE user_id=? AND account_type=?",
#                        (encrypt_balance(f"{new_balance:.2f}"), self.controller.current_username, account_type.upper()))
#
#         # Add transaction record
#         cursor.execute("""
#             INSERT INTO transactions (user_id, account_type, type, amount)
#             VALUES (?, ?, ?, ?)
#         """, (self.controller.current_username, account_type.upper(), "DEPOSIT", encrypt_balance(f"{amount:.2f}")))
#
#         conn.commit()
#         conn.close()
#         self.load_accounts()
...
#
#     def withdraw(self, account_type):
#         amount = CTkInputDialog(text=f"Enter withdrawal amount for {account_type}:").get_input()
#         if not amount:
#             return
#         try:
#             amount = float(amount)
...
#             INSERT INTO transactions (user_id, account_type, type, amount)
#             VALUES (?, ?, ?, ?)
#         """, (self.controller.current_username, account_type.upper(), "WITHDRAWAL", encrypt_balance(f"{amount:.2f}")))
#
#         conn.commit()
#         conn.close()
#         self.load_accounts()
...
#
#         if not rows:
#             ctk.CTkLabel(self.transactions_frame, text="No transactions yet.", font=("Arial", 14)).pack(pady=10)
#             return
#
#         for acc_type, t_type, amount_enc, timestamp in rows:
#             amount = float(decrypt_balance(amount_enc))
#             text = f"{timestamp} | {acc_type.capitalize()} | {t_type} | ${amount:,.2f}"
#             ctk.CTkLabel(self.transactions_frame, text=text, anchor="w").pack(fill="x", padx=10, pady=2)
# class HomePage(ctk.CTkFrame):
#     def __init__(self, parent, controller):
#         super().__init__(parent, fg_color="#2e2e2e")
#         self.controller = controller
...
#             username,  # pass username instead of numeric ID
#             self.controller.decrypt_balance
#         )
#
#         if not accounts:
#             no_account_label = ctk.CTkLabel(
...
#         # Force frame size to match right-side container
#         self.configure(width=900, height=500)
#         self.pack_propagate(False)  # prevent shrinking to fit children
#
#         # Title
#         title = ctk.CTkLabel(
#             self,
...
#         # Frame to hold account cards
#         self.accounts_frame = ctk.CTkFrame(self, fg_color="#3a3a3a")
#         self.accounts_frame.pack(pady=20, padx=20, fill="both", expand=True)  # expand to fill space
#
#         self.load_accounts()
#
#     def load_accounts(self):
...
#
#         self.accounts_frame = ctk.CTkFrame(self, fg_color="#3a3a3a")
#         self.accounts_frame.pack(pady=20, padx=20, fill="x")
#
#         self.load_accounts()
#
#     def load_accounts(self):
#         # Clear old widgets (important if page reloads)
#         for widget in self.accounts_frame.winfo_children():
#             widget.destroy()
#
#         if not self.controller.current_user_id:
...
#         balance_label.pack(anchor="w", padx=15, pady=(5, 10))
#
#     def open_account(self, user_id, account_types, encrypted_zero_balance):
#         conn = sqlite3.connect("bankapp.db")
#         c = conn.cursor()
...
#                 VALUES (?, ?, ?)
#             """, (user_id, acc_type, encrypted_zero_balance))
#
#         conn.commit()
#         conn.close()
...
class OpenAccountPage(ctk.CTkFrame):
    ...
    def open_account(self):
        ...
        try:
            ...
            #         INSERT INTO accounts (user_id, account_type, balance)
            #         VALUES (?, ?, ?)
            #     """, (user_id, acc_type, encrypted_balance))
            #     opened.append(acc_type)
            #
            # conn.commit()
            # conn.close()
...
#             variable=self.checking_var,
#             font=("Arial", 14),
#             command=self.update_open_button_state  # callback on toggle
#         )
#         self.checking_cb.pack(pady=(0, 10), anchor="w")
#
#         self.savings_cb = ctk.CTkCheckBox(
#             container,
#             text="Savings Account",
...
#             command=self.update_open_button_state  # callback on toggle
#         )
#         self.savings_cb.pack(pady=(0, 20), anchor="w")
#
#         # Open account button
#         self.open_btn = ctk.CTkButton(
#             container,
...
#         )
#         self.open_btn.pack(pady=(0, 20), fill="x")
#
#         # Status label
#         self.status_label = ctk.CTkLabel(container, text="", font=("Arial", 12))
#         self.status_label.pack(pady=(0, 10))
#
#     # Callback to enable/disable button
#     def update_open_button_state(self):
#         if self.checking_var.get() or self.savings_var.get():
#             self.open_btn.configure(state="normal")
#         else:
...
#     def open_account(self):
#         import sqlite3
#         from crypto_utils import encrypt_balance  # or call your local encrypt_balance
#
...
#             account_types.append("savings")
#
#         # INPUT VALIDATION: at least one selected (extra safety)
#         if not account_types:
#             self.status_label.configure(
#                 text="Select at least one account type.",
#                 text_color="red"
...
#             self.status_label.configure(
#                 text="Invalid user. Please log in again.",
#                 text_color="red"
#             )
#             return
#
...
#                 self.status_label.configure(
#                     text=f"Invalid account type selected: {acc_type}",
#                     text_color="red"
#                 )
#                 return
#
...
#                     text_color="green"
#                 )
#                 # Automatically refresh HomePage to show new accounts
#                 self.controller.frames[HomePage].load_accounts()
#             else:
#                 self.status_label.configure(
#                     text="Selected account(s) already exist.",
...
#
#         # Checkboxes for account types
#         self.checking_var = ctk.BooleanVar(value=True)
#         self.savings_var = ctk.BooleanVar(value=False)
#
#         self.checking_cb = ctk.CTkCheckBox(
#             container,
...
#         title.pack(pady=30)
#
#         self.account_var = ctk.StringVar(value="checking")
#
...
#             if cursor.fetchone():
#                 self.status_label.configure(
#                     text=f"{account_type.capitalize()} account already exists.",
#                     text_color="orange"
#                 )
#                 conn.close()
#                 return
...
#                 INSERT INTO accounts (user_id, account_type, balance)
#                 VALUES (?, ?, ?)
#             """, (user_id, account_type, encrypted_balance))
#
#             conn.commit()
#             conn.close()
#
#             self.status_label.configure(
#                 text=f"{account_type.capitalize()} account opened successfully!",
#                 text_color="green"
#             )
#
#         except Exception as e:
